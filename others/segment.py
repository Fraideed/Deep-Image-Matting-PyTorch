...
def vis_segmentation(image, seg_map):
    ...
    # new_in=Image.fromarray(seg_image)
    # new_in.save('seg.png')
    # plt.imshow(seg_image)
    # plt.axis('off')
    # plt.title('segmentation map')
    # gray = Image.open('seg.png').convert('L')
    # threshold = 5
    # table = []
    # for i in range(256):
    #     if i < threshold:
    ...
    # bim2=bim.resize(original_im.size)
    # bim2.save('binary.png')
    # bim2.show()
    ...
    # ret2,mask2=cv2.threshold(gray,0,255,cv2.THRESH_BINARY_INV)
    # output=image=cv2.add(ori, np.zeros(np.shape(ori), dtype=np.uint8), mask=mask)
    # output = cv2.bitwise_and(ori,np.zeros(np.shape(ori),dtype=np.uint8), mask=mask)
    ...
    # cv2.imshow('ori',ori)
    # output = cv2.add(ori, np.zeros(np.shape(ori), dtype=np.uint8), mask=mask)
